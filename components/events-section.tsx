...
const upcomingEvents = [
  {
    ...
    date: "April 15, 2026",
    location: "VVIET Main Campus",
    attendees: 500,
    type: "Reunion",
    image: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/campus%20pic-UgwoAr2O9DGuRQ8EEC0bu6lfmuIIW2.png",
    featured: true,
  },
  {
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          {upcomingEvents
            ...
            .map((event) => (
              <div
                key={event.title}
                className={`group relative overflow-hidden rounded-2xl border border-border bg-background shadow-sm transition-all duration-500 hover:shadow-xl lg:row-span-2 ${isVisible ? "animate-slide-up" : "opacity-0"}`}
              >
                <div className="relative h-56 overflow-hidden">
                  ...
                  <span className={`absolute top-4 left-4 rounded-lg px-3 py-1 text-xs font-semibold ${typeColors[event.type]}`}>
                    {event.type}
                  </span>
                </div>
                <div className="p-6">
          ...
          <div className="flex flex-col gap-4">
            {upcomingEvents
              ...
              .map((event, i) => (
                <div
                  key={event.title}
                  className={`group flex items-center gap-5 rounded-2xl border border-border bg-background p-5 shadow-sm transition-all duration-300 hover:shadow-lg hover:border-primary/20 ${isVisible ? "animate-slide-up" : "opacity-0"}`}
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="mb-1 flex items-center gap-2">
                      <span className={`rounded-md px-2 py-0.5 text-xs font-semibold ${typeColors[event.type]}`}>
                        {event.type}
                      </span>
                    </div>
                  ...
                  <button className="shrink-0 rounded-xl border border-primary/20 bg-primary/5 px-4 py-2 text-sm font-semibold text-primary transition-all hover:bg-primary hover:text-primary-foreground">
                    Register
                  </button>
                </div>
              ))}
